...
string CRPCTable::help(string strCommand) const
{
    ...
    for (map<string, const CRPCCommand*>::const_iterator mi = mapCommands.begin(); mi != mapCommands.end(); ++mi)
    {
        ...
        if (fAllAnon)
        {
            if (strMethod != "sendtpaytoanon"
                && strMethod != "sendanontoanon"
                && strMethod != "sendanontotpay"
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "Stop TokenPay server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "TokenPay server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe reqWallet
  //  ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "sendtpaytoanon",          &sendtpaytoanon,          false,     false,     false },
    { "sendanontoanon",         &sendanontoanon,         false,     false,     false },
    { "sendanontotpay",          &sendanontotpay,          false,     false,     false },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        std::string strWhatAmI = "To use tokenpayd";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"TokenPay Alert\" admin@foo.com\n"),
                strWhatAmI,
